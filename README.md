...
# Использование: ./run_ldrgen.sh [доп. флаги ldrgen]
# Пример: ./run_ldrgen.sh -ldrgen-seed 12345

LDRGEN_CMXS="./ldrgen.cmxs"
OUTPUT="generated.c"
BINARY="./generated_out"

# Генерируем функцию
echo "" > dummy.c
frama-c dummy.c -load-module "$LDRGEN_CMXS" -ldrgen "$@" > "$OUTPUT" 2>/dev/null

if [ ! -s "$OUTPUT" ]; then
    echo "Ошибка: ldrgen не сгенерировал код"
    exit 1
fi

# Извлекаем сигнатуру функции (строка с именем fn1)
SIGNATURE=$(grep -oP '^[^/\s][^\n]*fn1\s*\([^)]*\)' "$OUTPUT" | head -1)

if [ -z "$SIGNATURE" ]; then
    echo "Ошибка: не удалось найти сигнатуру функции"
    cat "$OUTPUT"
    exit 1
fi

# Извлекаем тип возврата
RET_TYPE=$(echo "$SIGNATURE" | grep -oP '^[\w\s]+(?=fn1)')
RET_TYPE=$(echo "$RET_TYPE" | xargs)  # trim пробелов

# Извлекаем параметры
PARAMS=$(echo "$SIGNATURE" | grep -oP '(?<=\().*(?=\))')

# Генерируем аргументы для вызова (нули подходящего типа)
# Для каждого параметра подставляем 0 или NULL для указателей
ARGS=""
if [ -n "$PARAMS" ] && [ "$PARAMS" != "void" ]; then
    ...
    for PARAM in "${PARAM_LIST[@]}"; do
        ...
        if echo "$PARAM" | grep -q '\*'; then
            # Указатель — передаём NULL
            ARGS="${ARGS}NULL, "
        else
            # Скалярный тип — передаём 0
            ARGS="${ARGS}0, "
        fi
    done
...
# Добавляем main в конец файла
cat >> "$OUTPUT" << EOF

#include <stdio.h>

int main(void) {
    ${RET_TYPE} result = fn1(${ARGS});
    printf("Result: %lld\n", (long long)result);
    return 0;
}
EOF
